...
| Change the stats committee dashboard (build health, season coverage, games played but not entered) | `committees/stats/index.html` — one self-contained file, public. Reads `GET /api/build/status`, `GET /api/schedule` and `GET /api/boxscores/games`; the queue card and the Trigger-a-build button appear only for the `stats` role (or `admin`) and read `GET /api/boxscore/pending` / `/queue`. A scheduled game is matched to a box score on **date plus its two teams unordered**, so a game entered with the sides swapped doesn't read as missing. Deliberately **not** in `tests/frontend/run.js`: its one countable list is empty in the offseason and empty again whenever the committee is caught up |
