...
| Change the schedule page (public, read-only) | `schedule/index.html` — one self-contained file; data from `GET /api/schedule` + `/api/schedule/seasons` (`nbn-api/routers/schedule.py`). "Upcoming" means **no box score exists for that fixture**, not that its date is future — the sim runs months behind the real calendar, so the date is the wrong question. Fixture edits are BOD-only and live on `/calendar`. This page makes no writes — it shows who's claimed a game (`streamer`) and whether it's flagged as one (`stream`), both read-only here; claiming/flagging happens on `/stream` |
| Change the streamer dashboard (game claims, the stream flag, or the Coaching Settings entry queue) | `stream/index.html` — one self-contained file, gated to the `streamer` role (and `admin`) at the page level; two tabs. "Claim Games" is the same season browser as `/schedule`, moved here because claiming/flagging are the only writes it makes: a streamer claim (`streamer`, who's broadcasting) and the stream flag (`stream`, whether the game itself is marked as one) — two independent fields, neither implies the other. "Coaching Settings" is the entry-queue dashboard for the row below |
