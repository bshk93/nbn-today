...
| Change the league-wide compliance board | The **Compliance tab** of `committees/rosters/index.html` — the first of four tabs, public and read-only. Its script is its own IIFE at the bottom of that file, separate from the reconciliation script above it, and its DOM ids are all `comp-`-prefixed so nothing collides. Rules: **`cap-health.js`**, the same module as the row above; data: `GET /api/cap-history/current` (`nbn-api/routers/cap_history.py`), which already returns that module's exact inputs for all 30 teams. **The page does no cap math and states no rule of its own** — it calls `CapHealth.standing()` once per team and arranges the answers, so a condition added to `cap-health.js` appears here without an edit. It cannot price a § 2.1a Empty Roster Charge (`computeEmptyRosterCharge` is trapped in `teams/team.js`, which injects a page on load); `standing()` supports being called without `erc` and names the floor instead. Covered by `tests/frontend/run.js` |
