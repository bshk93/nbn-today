...
> **A Python replacement exists and is verified, but is not live.** All 86
> files below also have an implementation in `nbn-api/stats_build/`, checked
> byte for byte against R (see `docs/stats-pipeline-port-spec.md` and
> `nbn-api/CLAUDE.md`). `build.sh` still runs `job.R`; switching over is a
> deliberate step (spec Phase 3) that wants a quiet window between games.
> Two bugs the Python version fixes are therefore still live in these files:
> `league-history.csv` lists every playoff team as a champion, and three
> players named Will are written `Barton, will`.
