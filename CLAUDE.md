...
| Change the Roster Settings section (jersey #, secondary position, player minutes) | `setupTeamSettingsTab` — `teams/team.js`. Folded into the Coaching tab (`#tab-coaching`) 2026-09-04, then merged 2026-09-04 with the Player Minutes depth chart that used to live in Coaching Profile below it — both are player-indexed tables ("list every roster player, edit something about them"), so they're now one table, one row per player, rather than two. **A slot is which row a player is in, not a field they pick** (changed 2026-09-04 from a per-row `<select>` of all 15 slots to this): table order *is* the depth chart, row 0 always `CS.MINUTES_SLOTS[0]` (`PG`), row 1 `SG`, … row 4 `C`, row 5 the 6th man, … row 14 the 15th man, and reordering is drag-and-drop (a `⠿` handle column, edit mode only — native HTML5 DnD, `tr.draggable` toggled on the handle's `mousedown`/`mouseup` rather than left on so the rest of the row keeps normal text selection). `orderedRows()` opens Edit with the roster already arranged in last-saved slot order (whoever's `minutes[slot].slug` matches first, in slot order, then anyone unassigned after) rather than raw roster order. Nothing is kept in sync as rows move — `minutesFromDom()` derives the whole slot-indexed `minutes` object fresh from live DOM order (row index → `CS.MINUTES_SLOTS[i]`) whenever it's needed (the running total, and the final save), which is also what makes a slot holding at most one player automatic (one DOM position, one row) instead of the exclusivity bookkeeping the old per-row dropdown needed. The stored shape itself is unchanged — still slot-indexed, in the *coaching-settings* record, not the player bio — since `stream/index.html`'s streamer dashboard reads that same shape directly and unmerged (`CS.renderReadOnly`'s own Player Minutes card, `opts.skipMinutesCard` left false there). Saving jersey/secondary-pos goes to `PUT /api/players/{slug}/team-settings` per changed player as before; saving minutes goes to `PUT /api/coaching-settings/{team}` — a full-replace-both-fields endpoint — so a minutes-only save always resends the team's current (unedited) `values` alongside the edited `minutes`, and a jersey-only save skips that request entirely (a `minutesDirty` flag decides), since PUTting there unconditionally sets `pending: true` and would falsely tell the streamer something coaching-related needs re-entering |
