...
| Change the Roster Settings section (jersey #, secondary position, player minutes) | `setupTeamSettingsTab` — `teams/team.js`. Folded into the Coaching tab (`#tab-coaching`) 2026-09-04, then merged 2026-09-04 with the Player Minutes depth chart that used to live in Coaching Profile below it — both are player-indexed tables ("list every roster player, edit something about them"), so they're now one table, one row per player, rather than two. Minutes are stored slot-indexed (`CS.MINUTES_SLOTS`: `PG SG SF PF C 6 7 ... 15`, one slot holds at most one slug) in the *coaching-settings* record, not the player bio — this table flips that lookup (`slotForSlug`) to render/edit it per player instead of per slot, without changing the stored shape, since `stream/index.html`'s streamer dashboard reads that same shape directly and unmerged (`CS.renderReadOnly`'s own Player Minutes card, `opts.skipMinutesCard` left false there). Saving jersey/secondary-pos goes to `PUT /api/players/{slug}/team-settings` per changed player as before; saving minutes goes to `PUT /api/coaching-settings/{team}` — a full-replace-both-fields endpoint — so a minutes-only save always resends the team's current (unedited) `values` alongside the edited `minutes`, and a jersey-only save skips that request entirely (a `minutesDirty` flag decides), since PUTting there unconditionally sets `pending: true` and would falsely tell the streamer something coaching-related needs re-entering |
